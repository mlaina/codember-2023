...
const resolved = (encrypted) => {
  const words = encrypted.toLowerCase().split(' ')
  const result = {}
  let message = ''
  ...
  words.forEach((word) => {
    if (!result[word]) {
      ...
      const newCount = ++result[word]
      message = message.replace(word + (newCount - 1), word + newCount)
    }
  })
